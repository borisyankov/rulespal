# Star Wars: X-Wing Miniatures Game – The Force Awakens Core Set Rules Reference
...
**Stop!** Read the Learn to Play booklet before reading this document. After reading the Learn to Play booklet, players are ready to play their first game. As questions arise during gameplay, players should reference this guide instead of the Learn to Play booklet.
...
7. **Deal Damage:** If the defender was hit, it suffers one damage for each uncanceled hit result and one critical damage for each uncanceled critical hit result. All damage must be suffered before any critical damage may be suffered.
...
* **Bank:** The left bank and right bank bearings advance a ship at a shallow curve to one side, changing its facing by 45º.
* **Turn:** The left turn and right turn bearings advance a ship at a tight curve to one side, changing its facing by 90º.
...
* **Segnor's Loop:** The Segnor's Loop left and Segnor's Loop right bearings advance a ship at a shallow curve to one side, changing its facing by 225º. To achieve this facing, the player slides the ship's front guides into the end of the maneuver template instead of the rear guides.
* **Tallon Roll:** The Tallon Roll left and Tallon Roll right bearings advance a ship at a tight curve to one side, sharply changing its facing by 180º. To achieve this facing, before the player places the ship at the opposite end of the template, he rotates the ship 90º to the left for a Tallon Roll left, or 90º to the right for a Tallon Roll right. The side of the ship's base must be placed flush against the end of the maneuver template, and the end of the template cannot extend beyond the front or back of the ship's base.
* **Stationary:** The stationary bearing does not have a corresponding maneuver template and is always paired with a speed value of "0" to indicate that the ship does not move from its current position.
* If a ship overlaps another ship while executing a Koiogran turn or a Segnor's Loop, the ship does not rotate 180º at the opposite end of the template. The player places the ship using the rear guides instead of the front guides.
* When a ship executes a Tallon Roll, it must avoid overlapping other ships if possible. If the ship cannot avoid overlapping another ship, it does not rotate 90º at the end of the template.
...
1. Choose the [straight 1], [left bank 1], or [right bank 1] maneuver template.
...
2. Perform a boost using the [straight 2] maneuver template.
...
* **Hit:** If this result is not canceled, the defender suffers one damage.
* **Critical:** If this result is not canceled, the defender suffers one critical damage.
* **Focus:** These results have no inherent effect, but the attacker may spend a focus token to change them to hit results.
* **Blank:** This result does not display any icons. It has no effect.

The results on the defense dice are as follows:

* **Evade:** This result cancels one hit or critical hit result.
* **Focus:** These results have no inherent effect, but the defender may spend a focus token to change them to evade results.
* **Blank:** This result does not display any icons. It has no effect.
...
* If a ship moves through or overlaps an obstacle while executing a Koiogran turn, Segnor's Loop left, or Segnor's Loop right maneuver, it still rotates 180º when placed.
* If a ship moves through or overlaps an obstacle while executing a Tallon Roll left or Tallon Roll right maneuver, it still rotates 90º when placed.
...
* The stationary bearing does not have a corresponding maneuver template and is always paired with a speed value of "0" to indicate that the ship does not move from its current position.
...
1. TIE/fo Fighter Ship Cards with Maneuver Dials, Upgrade Cards, and ID Tokens
2. First Order Edge
3. Dice
4. First Order Setup Area
5. Evade and Focus Tokens
6. Damage Deck
7. Asteroids
8. Stress Tokens
9. Range Ruler
10. Maneuver Templates
11. Critical Hit Tokens

*This setup diagram assumes that each player has a core set (split by faction) and ships from expansion packs or additional core sets.*
...
2. **Activation Phase:** In ascending order of pilot skill, each ship reveals its dial, executes its maneuvers, and may perform one action.
...
* **Range 1:** one additional attack die.
* **Range 3:** one additional defense die.
...
* **Boost:** Take the [straight 1], [left bank 1], or [right bank 1] template and move forward using the selected template.
* **Barrel Roll:** Take the [straight 1] template and move sideways (either left or right), facing the same direction.
* **Focus:** Place 1 focus token on the ship. During the Combat phase, spend it to change all of the ship's focus results to hit results (when attacking) or to evade results (when defending).
* **Evade:** Place 1 evade token on the active ship. During the Combat phase, spend it to add 1 evade result to the ship's defense roll.
* **Acquire a Target Lock:** Place 1 blue target lock token near the locking ship and 1 red target lock token near an enemy ship at Range 1–3. If the locking ship attacks the locked ship, the locking ship may spend the pair of tokens to reroll any number of its attack dice.

### Ship Maneuvers

The tables below list all possible maneuvers that a T-70 X-wing and a TIE/fo Fighter can execute. Players can reference these tables at any time. (The printed booklet shows these as maneuver dial diagrams for the T-70 X-wing and the TIE/fo Fighter.)

When a ship executes a red maneuver, place one stress token near it. While a ship is stressed, it cannot execute red maneuvers or perform actions.

When a ship executes a green maneuver, remove one stress token from it.
