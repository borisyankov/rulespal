...
- `api.geekdo.com` is reachable even when boardgamegeek.com HTML is Cloudflare-blocked and BGG's XML API 401s — 787 consecutive lookups in one survey with zero failures, so treat a miss here as a real "no file", not a transport problem. Metadata: `api/geekitems?objectid=<id>`. Files list (paginated, newest-first): `api/files?objectid=<id>&objecttype=thing&pageid=<n>` — page deeper if page 1 has nothing official, or add `&sort=hot&showcount=50`, which floats the actual rulebook to the top of page 1. Each entry carries `language`, `filename` and `size`; **the filename is the best search key you'll get** for finding the same PDF on a mirror once the gated download fails. Images: `api/images?objectid=<id>&objecttype=thing` (use the signed thumbor URL from the response; the bare `cf.geekdo-images.com/…/__original/…` form often 400s).
...
- `api.tesera.ru/games/<slug>/files` → `tesera.ru/images/items/<id>/<filename>` — a Russian-community mirror that has repeatedly had byte-exact official scans for Oink Games and OOP/small-press titles other routes couldn't reach. **Don't guess the slug**: `api.tesera.ru/search/games?query=<title>` returns candidate aliases, and `api.tesera.ru/games/<alias>` carries a `bggId` field — check it against the game's real id before trusting the match, since the site titles games in Russian (Courtisans is listed as "Придворные"). The PDF URLs it returns download with a plain `curl`, no gate. Surveyed over 787 rank 1000-2000 games: 80% had a Tesera page, 62% a PDF, 26% an **English** one — so it is worth trying well before the translation routes, not just as a Russian-language last resort.
